    ...
    nim = {
      ['textDocument/completion'] = { --{{{
        ...
        handler = function(_, params, done)
          local p = vim.lsp.protocol
          local symbolKinds = {
            skConditional = p.CompletionItemKind.Operator,
            skParam = p.CompletionItemKind.Variable,
            skGenericParam = p.CompletionItemKind.TypeParameter,
            skModule = p.CompletionItemKind.Module,
            skType = p.CompletionItemKind.Class,
            skVar = p.CompletionItemKind.Variable,
            skLet = p.CompletionItemKind.Value,
            skConst = p.CompletionItemKind.Constant,
            skResult = p.CompletionItemKind.Keyword,
            skProc = p.CompletionItemKind.Function,
            skFunc = p.CompletionItemKind.Function,
            skMethod = p.CompletionItemKind.Method,
            skIterator = p.CompletionItemKind.Method,
            skConverter = p.CompletionItemKind.Method,
            skMacro = p.CompletionItemKind.Function,
            skTemplate = p.CompletionItemKind.Function,
            skField = p.CompletionItemKind.Field,
            skEnumField = p.CompletionItemKind.EnumMember,
            skForVar = p.CompletionItemKind.Keyword,
            skLabel = p.CompletionItemKind.Value,
          }
            ...
            'sug',
            {
              on_data = function(reply)
                for _, item in ipairs(reply) do
                  ...
                  if parts[1] == 'sug' then
                    local kind = symbolKinds[parts[2]] or p.CompletionItemKind.Text
                    local name = parts[3]
                    local name_parts = vim.split(name, '.', { plain = true })
                    local label = name_parts[#name_parts]
                    local description = vim.fn.eval(parts[8])
                    local editFormat = p.InsertTextFormat.PlainText
                    local textEdit
                    if kind == p.CompletionItemKind.Function or
                      kind == p.CompletionItemKind.Method
                    then
                      local param_count = 0
      ...
      ['textDocument/didChange'] = { --{{{
        handler = function(message_id, params)
          local did_change = lsp_info.servers.nim['textDocument/didChange']

          local path = uri_to_path(params.textDocument.uri)
          local diag_stack = lsp_info.diag_stack.nim[path]
          ...
          if diag_stack and #diag_stack > 0 then
            ...
            vim.fn['nim#suggest#utils#Query'](
              'chk',
              {
                buffer = buf,
                on_data = function(reply)
                  if #lsp_info.diag_stack.nim[path] == 1 then
                    local diagnostics = {}
                    for _, item in ipairs(reply) do
                      local parts = vim.split(item, '\t',
                        { plain = true, trimempty = false })
                ...
                on_end = function()
                  local stack_count = #lsp_info.diag_stack.nim[path]
                  if stack_count > 1 then
                    local last_task = lsp_info.diag_stack.nim[path][stack_count]
                    lsp_info.diag_stack.nim[path] = {}
      ...
      ['textDocument/didOpen'] = { --{{{
        handler = function(message_id, params)
          ...
          local nim_inst = vim.fn['nim#suggest#ProjectFindOrStart']()
          if type(nim_inst) == 'table'
              and type(nim_inst.file) == 'string'
          then
            local nim_proj = nim_inst.file
            if not lsp_info.proj_cache.nim[nim_proj] then
              local buf = get_or_open_buf(nim_proj)
              local ns = vim.api.nvim_create_namespace('nim_lsp')
              ...
              vim.fn['nim#suggest#utils#Query'](
                'chk',
                {
                  buffer = buf,
                  on_data = function(reply)
                    if #lsp_info.diag_stack.nim == 1 then
                      local diagnostics = {}
                      for _, item in ipairs(reply) do
                        local parts = vim.split(item, '\t',
                          { plain = true, trimempty = false })
                  ...
                  on_end = function()
                    ...
                    lsp_info.diag_stack.nim[path] = vim.list_extend(
                      diag_stack or {},
                      lsp_info.diag_stack.nim[path])
                  end,
                },
                false,
                true)
              lsp_info.proj_cache.nim[nim_proj] = true
            else
              did_change.handler(message_id, params)
            end
          else
...
local function get_lsp_capabilities(server)
  local capabilities = {
    textDocumentSync = {
  ...
  for method, def in pairs(server) do
...
function fn.nim_lsp()
  ...
  local function handler(method, params, callback, notify_callback)
    ...
    if method == 'initialize' then
      send{ capabilities = get_lsp_capabilities(lsp_info.servers.nim) }
    elseif method == 'shutdown' then
      is_stopped = true
      send()
    elseif method == 'exit' then
      vim.fn['nim#suggest#ProjectStopAll']()
      lsp_info.proj_cache.nim = {}
      send()
    else
      local def = lsp_info.servers.nim[method]
